...
###   This script attempts to download the signature file SHA256SUMS.asc from chratos.org
...
WORKINGDIR="/tmp/chratos"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://chratos.org/bin/"
VERSIONPREFIX="chratos-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'chratos-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Chratos Core binary release signing key installed?"
   fi
