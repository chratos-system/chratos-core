...
The RPM spec file provided here is for Chratos-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://chratos.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Chratos git repository](https://github.com/chratos/chratos) but are not part of
...
Currently only `Source10` is used, for the example `chratos.conf` file.
...
in the chratos git repository. At present only an SVG version of the Chratos
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the chratos
...
needed when 0.12.1 is released, a proper fix is already in the Chratos git
master branch.
...
idea to build Chratos Core against a static version of that library compiled
...
The `chratos.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `chratos-server` package. At this point, most RPM based
...
The `chratos-server` package should have SELinux support. How to properly do
...
Chratos-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/chratos/). He has been
packaging Chratos for Fedora at least since 2012.
...
4. I renamed the `chratos` package that contains the Qt GUI to `chratos-core` as
that appears to be how the general population refers to it, in contrast to
`chratos-xt` or `chratos-classic`. I wanted to make sure the general population
