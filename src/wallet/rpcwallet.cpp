...
UniValue getnewaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Chratos address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"chratosaddress\"    (string) The new chratos address\n"
            "\nExamples:\n"
    ...
    return CChratosAddress(keyID).ToString();
}


CChratosAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CChratosAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Chratos address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"chratosaddress\"   (string) The account chratos address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Chratos address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            "setaccount \"chratosaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"chratosaddress\"  (string, required) The chratos address to be associated with an account.\n"
    ...
    CChratosAddress address(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Chratos address");
...
UniValue getaccount(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            "getaccount \"chratosaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"chratosaddress\"  (string, required) The chratos address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "  \"chratosaddress\"  (string) a chratos address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CChratosAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CChratosAddress& address = item.first;
...
UniValue sendtoaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"chratosaddress\" amount ( \"comment\" \"comment-to\" \"anon-destination\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"chratosaddress\"  (string, required) The chratos address to send to.\n"
            ...
            "5. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a CHRtech transaction \n"
            "6. subtractfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            "                             The recipient will receive less chratoss than you enter in the amount field.\n"
            "\nResult:\n"
    ...
    if(DNS->check_address_syntax(request.params[0].get_str().c_str()))
    {
        bool dnssec_valid;
        std::vector<std::string> addresses = utils::dns_utils::addresses_from_url(request.params[0].get_str().c_str(), dnssec_valid);
    ...
    CChratosAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Chratos address");
    ...
    if (request.params.size() > 4 && !request.params[4].isNull() && !request.params[4].get_str().empty())
        strDZeel = request.params[4].get_str();
...
UniValue stakervote(const JSONRPCRequest &request)
{
    if (!EnsureWalletIsAvailable(request.fHelp))
        return NullUniValue;
    ...
    SoftSetArg("-stakervote", request.params[0].get_str());
...
UniValue createproposal(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 4)
        throw runtime_error(
            ...
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "CHR is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"chratosaddress\"     (string, required) The chratos address where coins would be sent if proposal is approved.\n"
    ...
    CChratosAddress destaddress(Address);
...
UniValue createpaymentrequest(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 CHR\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CChratosAddress address(proposal.Address);

    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid Chratos address.");
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nReqAmount) + "CHR from the proposal " + proposal.hash.ToString() + ". Payment request id: " + id;
...
UniValue listaddressgroupings(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"chratosaddress\",     (string) The chratos address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CChratosAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CChratosAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CChratosAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw runtime_error(
            "signmessage \"chratosaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"chratosaddress\"  (string, required) The chratos address to use for the private key.\n"
    ...
    CChratosAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"chratosaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given chratosaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"chratosaddress\"  (string, required) The chratos address for transactions.\n"
    ...
    // Chratos address
    CChratosAddress address = CChratosAddress(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Chratos address");
...
UniValue sendfrom(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tochratosaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a chratos address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tochratosaddress\"  (string, required) The chratos address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(request.params[0]);
    CChratosAddress address(request.params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Chratos address");
    CAmount nAmount = AmountFromValue(request.params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The chratos address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less chratoss than you enter in their corresponding amount field.\n"
    ...
    set<CChratosAddress> setAddress;
    vector<CRecipient> vecSend;
    ...
    {
        CChratosAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Chratos address: ")+name_);
...
extern CScript _createmultisig_redeemScript(const JSONRPCRequest &request);
...
UniValue addmultisigaddress(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Chratos address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of chratos addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) chratos address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"chratosaddress\"  (string) A chratos address associated with the keys.\n"
    ...
    CScript inner = _createmultisig_redeemScript(request);
    CScriptID innerID(inner);
    pwalletMain->AddCScript(inner);

    pwalletMain->SetAddressBook(innerID, strAccount, "send");
    return CChratosAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const JSONRPCRequest &request)
{
    ...
    return CChratosAddress(w.result).ToString();
}
...
UniValue ListReceived(const JSONRPCRequest &request, bool fByAccounts)
{
    // Minimum confirmations
    int nMinDepth = 1;
    if (request.params.size() > 0)
    ...
    // Tally
    map<CChratosAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CChratosAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CChratosAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listreceivedbyaddress(const JSONRPCRequest &request)
{
    ...
    return ListReceived(request, false);
}
...
UniValue listreceivedbyaccount(const JSONRPCRequest &request)
{
    ...
    return ListReceived(request, true);
}

static void MaybePushAddress(UniValue & entry, const CTxDestination &dest)
{
    CChratosAddress addr;
    if (addr.Set(dest))
...
UniValue listtransactions(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"chratosaddress\",    (string) The chratos address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"chratosaddress\",    (string) The chratos address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"chratosaddress\",   (string) The chratos address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest &request)
{
    ...
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending chratoss\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest &request)
{
    ...
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending chratos\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"chratosaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Chratos server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending chratoss.\n"
...
UniValue listunspent(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of chratos addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) chratos address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the chratos address\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CChratosAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Chratos address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CChratosAddress(address).ToString()));
...
UniValue fundrawtransaction(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The chratos address to receive the change\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CChratosAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid chratos address");
...
#ifdef HAVE_UNBOUND
UniValue resolveopenalias(const JSONRPCRequest &request)
{
  ...
  if ((request.fHelp || request.params.size() != 1))
      throw runtime_error(
          ...
          "\nResolves the give OpenAlias address to a Chratos address.\n"
          "\nArguments:\n"
...
{ //  category              name                        actor (function)           okSafeMode   argNames
    //  --------------------- ------------------------    -----------------------    ---------- --------
    { "rawtransactions",    "fundrawtransaction",       &fundrawtransaction,       false, {"hexstring","options","iswitness"} },
    { "hidden",             "resendwallettransactions", &resendwallettransactions, true,  {} },
    { "wallet",             "abandontransaction",       &abandontransaction,       false, {"txid"} },
    { "wallet",             "addmultisigaddress",       &addmultisigaddress,       true,  {"nrequired","keys","label|account","address_type"} },
    ...
    { "wallet",             "getnewaddress",            &getnewaddress,            true,  {"label|account","address_type"} },
    ...
    { "wallet",             "importaddress",            &importaddress,            true,  {"address","label","rescan"} },
    { "wallet",             "importprunedfunds",        &importprunedfunds,        true,  {"rawtransaction","txoutproof"} },
    { "wallet",             "importpubkey",             &importpubkey,             true,  {"pubkey","label","rescan","p2sh"} },
    { "wallet",             "keypoolrefill",            &keypoolrefill,            true,  {"newsize"} },
    { "wallet",             "listaccounts",             &listaccounts,             false, {} },
    { "wallet",             "listaddressgroupings",     &listaddressgroupings,     false, {} },
    { "wallet",             "listlockunspent",          &listlockunspent,          false, {} },
    { "wallet",             "listreceivedbyaccount",    &listreceivedbyaccount,    false, {} },
    { "wallet",             "listreceivedbyaddress",    &listreceivedbyaddress,    false, {"minconf","include_empty","include_watchonly","address_filter"} },
    { "wallet",             "listsinceblock",           &listsinceblock,           false, {"blockhash","target_confirmations","include_watchonly","include_removed"} },
    { "wallet",             "listtransactions",         &listtransactions,         false, {"account|dummy","count","skip","include_watchonly"} },
