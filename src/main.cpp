...
bool CScriptCheck::operator()() {
    ...
    if (ptxTo->IsCoinStake() && nIn == 0) { 
      return scriptSig.GetSigOpCount(true) == 0;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, bool fProofOfStake)
{
    ...
    pindex->nDividendFund = pindex->pprev != NULL ? pindex->pprev->nDividendFund : 0;
    ...
    */
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        BOOST_FOREACH(const CTxOut& vout, tx.vout) {
          if(vout.IsDividendContribution()) {
            pindex->nDividendFund += vout.nValue;
          }
        }
...
//
// Commented - Chratos uses now version control
//
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev, bool fProofOfStake)
{
    ...
    if (block.IsProofOfStake() && nHeight < Params().GetConsensus().nFirstPOSBlock)
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of Chratos, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of Chratos, please update.";
            fObsolete = true;
        }
