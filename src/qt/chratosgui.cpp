...
#include "config/chratos-config.h"
#endif
...
const std::string ChratosGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ChratosGUI::DEFAULT_WALLET = "~Default";

ChratosGUI::ChratosGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void ChratosGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/chratos"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Chratos address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and chratos: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Chratos addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Chratos addresses"));
    ...
    openAction->setStatusTip(tr("Open a chratos: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Chratos command-line options").arg(tr(PACKAGE_NAME)));
...
void ChratosGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(ChratosUnits::Unit u, ChratosUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(ChratosUnits::name(u)), this);
...
void ChratosGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void ChratosGUI::createToolBars()
{
    if(walletFrame)
    {
...
void ChratosGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool ChratosGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void ChratosGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool ChratosGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ChratosGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ChratosGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ChratosGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ChratosGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ChratosGUI::optionsClicked()
{
...
void ChratosGUI::showDebugWindow()
{
...
void ChratosGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ChratosGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ChratosGUI::openClicked()
{
...
void ChratosGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void ChratosGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ChratosGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ChratosGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ChratosGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void ChratosGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void ChratosGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Chratos network", "", count));
    if(walletFrame){
...
void ChratosGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to Chratos network."));
    }
    else
    {
...
void ChratosGUI::showVotingDialog()
{
...
void ChratosGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Chratos"); // default title
...
void ChratosGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ChratosGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ChratosGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void ChratosGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ChratosGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ChratosGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ChratosGUI::setEncryptionStatus(int status)
{
...
void ChratosGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ChratosGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ChratosGUI::detectShutdown()
{
...
void ChratosGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ChratosGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ChratosGUI::subscribeToCoreSignals()
{
...
void ChratosGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<ChratosUnits::Unit> units = ChratosUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const ChratosUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(ChratosUnits::name(unit)));
    }
...
{
    setText(ChratosUnits::name(newUnits));
}
...
void ChratosGUI::replyFinished(QNetworkReply *reply)
{
...
void ChratosGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CChratosAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void ChratosGUI::getVotingInfo()
{
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.chratos.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void ChratosGUI::updateStakingStatus()
{
    updateWeight();
