...
DividendView::DividendView(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent), model(0), transactionProxyModel(0),
    dividendView(0), abandonAction(0)
{
    // Build filter row
...
void DividendView::setModel(WalletModel *model)
{
    this->model = model;
    if(model)
    {
        ...
        dividendView->setColumnWidth(TransactionTableModel::Watchonly, WATCHONLY_COLUMN_WIDTH);
...
void DividendView::chooseWatchonly(int idx)
{
    if(!transactionProxyModel)
        return;
...
void DividendView::editLabel()
{
    if(!dividendView->selectionModel() ||!model)
        return;
...
// show/hide column Watch-only
void DividendView::updateWatchOnlyColumn(bool fHaveWatchOnly)
{
    watchOnlyWidget->setVisible(fHaveWatchOnly);
    dividendView->setColumnHidden(TransactionTableModel::Watchonly, !fHaveWatchOnly);
}
