...
const char * const CHRATOS_CONF_FILENAME = "chratos.conf";
const char * const CHRATOS_PID_FILENAME = "chratos.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Chratos
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Chratos
    // Mac: ~/Library/Application Support/Chratos
    // Unix: ~/.chratos
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Chratos";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Chratos";
#else
    // Unix
    return pathRet / ".chratos";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", CHRATOS_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No chratos.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override chratos.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", CHRATOS_PID_FILENAME));
