...
bool CFund::IsValidPaymentRequest(CTransaction tx)
{
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nAmount) + "CHR from the proposal " + proposal.hash.ToString() + ". Payment request id: " + strDZeel;

    CChratosAddress addr(proposal.Address);
    CKeyID keyID;
    addr.GetKeyID(keyID);
