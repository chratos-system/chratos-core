        ...
        if (tx.IsCoinBase())
            ...
            if (GetSpentIndex(spentKey, spentInfo)) {
                ...
                if (spentInfo.addressType == 1) {
                    in.push_back(Pair("address", CChratosAddress(CKeyID(spentInfo.addressHash)).ToString()));
                } else if (spentInfo.addressType == 2)  {
                    in.push_back(Pair("address", CChratosAddress(CScriptID(spentInfo.addressHash)).ToString()));
                }
            }
...
UniValue getrawtransaction(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            ...
            "           \"chratosaddress\"        (string) chratos address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue createrawtransaction(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the chratos address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            "      \"data\": \"hex\",     (string, required) The key is \"data\", the value is hex encoded data\n"
            "      ...\n"
            "    }\n"
            "3. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a CHRtech transaction \n"
    ...
    RPCTypeCheck(request.params, boost::assign::list_of(UniValue::VARR)(UniValue::VOBJ)(UniValue::VSTR)(UniValue::VNUM), true);
...
UniValue decoderawtransaction(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) chratos address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const JSONRPCRequest &request)
{
    ...
    r.push_back(Pair("p2sh", CChratosAddress(CScriptID(script)).ToString()));
    return r;
}
