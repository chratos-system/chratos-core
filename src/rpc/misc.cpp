...
UniValue validateaddress(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            "validateaddress \"chratosaddress\"\n"
            "\nReturn information about the given chratos address.\n"
            "\nArguments:\n"
            "1. \"chratosaddress\"     (string, required) The chratos address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"chratosaddress\", (string) The chratos address validated\n"
 ...
 */
CScript _createmultisig_redeemScript(const JSONRPCRequest &request)
{
...
#ifdef ENABLE_WALLET
        // Case 1: Chratos address and we have full public key:
        CChratosAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are chratos addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) chratos address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CChratosAddress address(innerID);
...
UniValue createwitnessaddress(const JSONRPCRequest &request)
{
    ...
    CChratosAddress address(witscriptid);
...
UniValue verifymessage(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            "verifymessage \"chratosaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"chratosaddress\"  (string, required) The chratos address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CChratosAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CChratosAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromRequest(const JSONRPCRequest &request,
    ...
    if (request.params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CChratosAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getaddressmempool(const JSONRPCRequest &request)
{
    ...
    if (!getAddressesFromRequest(request, addresses)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
