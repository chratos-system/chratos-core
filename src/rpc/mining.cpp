...
UniValue generatetoaddress(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
            "2. address    (string, required) The address to send the newly generated chratos to.\n"
...
// NOTE: Unlike wallet RPC (which use CHR values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/chratos/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/chratos/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Chratos is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Chratos is downloading blocks...");
...
UniValue submitblock(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.chratos.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatesmartfee(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in CHR)\n"
