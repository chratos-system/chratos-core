...
UniValue getaddednodeinfo(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:5556\",  (string) The chratos server IP and port we're connected to\n"
...
UniValue addanonserver(const JSONRPCRequest &request)
{
    string strCommand;
    if (request.params.size() >= 2)
        strCommand = request.params[1].get_str();
    if (request.fHelp || request.params.size() > 3 ||
        ...
        throw runtime_error(
            ...
            "\nAttempts add or remove a node from the CHRTech server list.\n"
            "\nArguments:\n"
            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
    ...
    if (strCommand == "add")
    {
      if (request.params.size() == 3 && request.params[2].get_str() == "true") {
        WriteConfigFile("addanonserver", strNode);
        if (it == vAddedAnonServers.end())
          vAddedAnonServers.push_back(strNode);
      } else {
        if (it != vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_ALREADY_ADDED, "Error: CHRTech server already added");
        vAddedAnonServers.push_back(strNode);
      }
    }
    else if(strCommand == "remove")
    {
      if (request.params.size() == 3 && request.params[2].get_str() == "true") {
        ...
        if (it == vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: CHRTech server has not been added.");
        vAddedAnonServers.erase(it);
      }
    }
    return NullUniValue;
...
UniValue listanonservers(const JSONRPCRequest &request)
{
...
UniValue getstakesubsidy(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "CHR decode failed");
    }
